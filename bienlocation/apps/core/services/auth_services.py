...
from bienlocation.apps.common.exceptions import AccountDeactivatedError, UserAlreadyExistsError
from bienlocation.apps.common.services import model_update
from bienlocation.apps.common.types import EmailDataType, UserRegistrationDataType
from bienlocation.apps.common.utils import tokenGenerator
from bienlocation.apps.core.models import Email, User
from bienlocation.apps.core.selectors.user_selectors import UserSelectors
from bienlocation.apps.core.services.email_services import EmailServices
from bienlocation.apps.core.services.user_services import UserServices
from bienlocation.apps.core.tasks.email_tasks import email_send as email_send_task
...
class AuthService:
    ...
    def login(self, email: str, password: str) -> User | None:
        ...
        if user and not user.email_confirmed:
            raise AccountDeactivatedError("Please activate your account first")

        return user

    @transaction.atomic
    def activate(self, uidb64: str, token: str) -> User | None:
        try:
        ...
        if user and tokenGenerator.check_token(user, token):
            ...
            context = {}
            self._email_send(payload=email_payload, template_name=self.template_email_activation_success, context=context)
            return user
        return None
    ...
    def password_reset_confirm(self, uidb64: str, token: str, payload: dict) -> None | User:
        try:
            public_id = force_str(urlsafe_base64_decode(uidb64))
            user = UserSelectors.get_user_by_public_id(public_id=public_id)
        ...
        if not user and not PasswordResetTokenGenerator().check_token(user, token):
        ...
        context = {}
        print()
        print("templete name: ", self.template_email_reset_password_complete)
        print()
