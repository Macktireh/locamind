...
class PasswordResetConfirmView(View):
    ...
    def get(self, request: HttpRequest, uidb64: str, token: str) -> HttpResponse:
        user = auth_service.password_reset_confirm_form(uidb64=uidb64, token=token)
        if user is None:
    ...
    def post(
        ...
        if form.is_valid():
            try:
                auth_service.password_reset_confirm(uidb64=uidb64, token=token, payload=form.cleaned_data)
